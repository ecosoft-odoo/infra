...
docker exec -it $postgres_container dropdb -U $postgres_user $database_test

# Copy prod db > test db
echo "Copy " $database_prod " to " $database_test
docker exec -it $postgres_container createdb -U $postgres_user $database_test --template=$database_prod

# disable scheduled actions for test db
echo "Disable all scheduled actions"
docker exec -it $postgres_container psql -U $postgres_user $database_test -c "update ir_cron set active = False"
